 ...
 * Читает закреплённое сообщение бота — ESP32 каждую минуту
 * пишет туда JSON со всеми данными.
 * Работает из любой точки мира через api.telegram.org
 */
const TG_BOT_TOKEN = '8899706883:AAEhg-tV6EN6j0HbBzMsdc1HIXoUsghXMWI';
const TG_CHAT_ID   = '5741848306';
...
class TelegramStatusService {
  ...
  async _fetch() {
    try {
      const url = `https://api.telegram.org/bot${TG_BOT_TOKEN}/getChat?chat_id=${TG_CHAT_ID}`;
      ...
      if (this.failCount > 0) {
        this.state.addLog(`Связь восстановлена: ${this.state.tempAir}°C, ${this.state.humidity}%`, 'esp');
      }
      this.failCount = 0;
      this.connected = true;
      ...
      if (this.failCount === this.MAX_FAILS) {
        this.state.addLog(`Telegram не отвечает: ${err.message}`, 'err');
        this.stopPolling();
